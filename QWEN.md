...
这是一个复刻原神中七圣召唤卡牌游戏的开源项目，实现了完整的卡牌对战功能，包括519张卡牌数据、完整的战斗系统、元素反应机制和卡组构筑功能。项目采用Flask作为后端框架，提供完整的REST API接口，支持用户认证、卡牌管理、卡组构建和游戏对战功能。前端使用React+TypeScript实现，并支持主题切换功能，提升用户体验。
...
- **包管理**: uv
- **前端**: React + TypeScript, 嵌入式HTML测试页面
- **测试**: pytest
...
- 数据库初始化使用 `db_manager.create_tables(app)` 方法确保模型正确初始化
