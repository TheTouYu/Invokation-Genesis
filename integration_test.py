...
def run_integration_tests():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if "cards" in characters_data and isinstance(
    ...
    try:
        response = requests.get(
            f"{base_url}/api/cards/events?per_page=3", headers=headers
        )
        if response.status_code == 200:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if len(character_cards) < 3:
                ...
                print(
                    f"  已获取 {len(character_cards)} 张角色卡: {[c[:8] for c in character_cards]}..."
                )  # 显示ID前8位

                # 获取足够多的卡来筛选非角色卡
                response = requests.get(
                    f"{base_url}/api/cards?per_page=100", headers=headers
                )
                if response.status_code == 200:
                    ...
                    # 从所有卡中筛选出非角色卡
                    for card in all_cards_data.get("cards", []):
                        if (
                            card["id"] not in character_cards
                            and card["card_type"] != "角色牌"
                            and len(other_cards) < 27
                        ):
                    ...
                    if len(other_cards) < 27:
                        ...
                        # 明确地将3张角色卡和27张其他卡组合成30张卡
                        all_deck_cards = (
                            character_cards[:3] + other_cards[:27]
                        )  # 确保只取3张角色卡和27张其他卡
                        ...
                        if len(all_deck_cards) != 30:
                            ...
                            if response.status_code == 201:
                                ...
                                if "deck" in create_deck_data:
                                    ...
                                    print(
                                        f"✗ 卡组创建返回非预期数据: {create_deck_data}"
                                    )
                                    return False
                            else:
                                print(
                                    f"✗ 卡组创建返回错误状态码: {response.status_code}"
                                )
                                print(f"  错误信息: {response.text}")
                                # 尝试调试：检查这些卡的类型
                                print("  调试信息: 正在检查卡牌类型...")
                                for i, card_id in enumerate(
                                    all_deck_cards[:5]
                                ):  # 检查前5张
                                    # 简单验证每张卡的类型
                                    print(
                                        f"    卡牌 {i + 1} ({card_id[:8]}...): 等待创建后验证类型"
                                    )
                                return False
                else:
                    print(f"✗ 获取卡牌用于创建卡组时失败: {response.status_code}")
                    return False
        else:
