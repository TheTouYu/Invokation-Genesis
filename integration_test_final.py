...
def get_non_character_cards(headers, base_url, num_needed, exclude_ids):
    ...
    while len(other_cards) < num_needed:
        response = requests.get(f"{base_url}/api/cards?per_page={per_page}&page={page}", headers=headers)
        ...
        for card in cards:
            if (card['id'] not in exclude_ids and 
                card['card_type'] != '角色牌' and 
                card['id'] not in other_cards and 
                len(other_cards) < num_needed):
...
def run_integration_tests():
    ...
    print("\n步骤 4: 测试获取角色卡...")
    try:
        response = requests.get(f"{base_url}/api/cards/characters?per_page=3", headers=headers)
        if response.status_code == 200:
            characters_data = response.json()
            if 'cards' in characters_data and isinstance(characters_data['cards'], list):
                print(f"✓ 获取角色卡测试通过 (获取到 {len(characters_data['cards'])} 张角色卡)")
            else:
    ...
    print("\n步骤 5: 测试获取事件卡...")
    try:
        response = requests.get(f"{base_url}/api/cards/events?per_page=3", headers=headers)
        if response.status_code == 200:
            events_data = response.json()
            if 'cards' in events_data and isinstance(events_data['cards'], list):
                print(f"✓ 获取事件卡测试通过 (获取到 {len(events_data['cards'])} 张事件卡)")
            else:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if len(character_cards) < 3:
                ...
                # 获取27张非角色卡
                other_cards = get_non_character_cards(headers, base_url, 27, character_cards)
                
                print(f"  已获取 {len(other_cards)} 张非角色卡")
                
                if len(other_cards) < 27:
                    print(f"⚠️  警告: 获取的非角色卡数量不足27张（当前有{len(other_cards)}张），跳过卡组创建测试")
                    deck_id = None
                else:
                    # 组合30张卡（3张角色卡 + 27张其他卡）
                    ...
                    if len(all_deck_cards) != 30:
                        print(f"⚠️  警告: 组合的卡牌数量不正确（当前有{len(all_deck_cards)}张），跳过卡组创建测试")
                        deck_id = None
                    else:
                        print(f"  正在创建包含 {len(character_cards[:3])} 张角色卡和 {len(other_cards[:27])} 张其他卡的卡组...")
                        ...
                        response = requests.post(f"{base_url}/api/decks", headers=headers, json=deck_data)
                        if response.status_code == 201:
                            ...
                            if 'deck' in create_deck_data:
                                ...
                                # Step 8: 测试获取特定卡组
                                print("\n步骤 8: 测试获取特定卡组...")
                                response = requests.get(f"{base_url}/api/decks/{deck_id}", headers=headers)
                                if response.status_code == 200:
                                    deck_info = response.json()
                                    if 'deck' in deck_info:
                                        print(f"✓ 获取特定卡组测试通过 (卡组: {deck_info['deck']['name']})")
                                    else:
                                        print(f"✗ 获取特定卡组返回非预期数据: {deck_info}")
                                        return False
                                else:
                                    print(f"✗ 获取特定卡组返回错误状态码: {response.status_code}")
                                    print(f"  错误信息: {response.text}")
                                    return False
                            else:
            ...
            if 'decks' in decks_data and isinstance(decks_data['decks'], list):
                if 'deck_id' in locals() and deck_id:  # 如果前面成功创建了卡组
                    if len(decks_data['decks']) > 0:
                        print(f"✓ 获取卡组验证测试通过 (用户现在有 {len(decks_data['decks'])} 个卡组)")
                    else:
                        print("✗ 卡组创建后用户卡组列表仍为空")
                        return False
                else:
                    print(f"✓ 获取卡组验证测试通过 (用户有 {len(decks_data['decks'])} 个卡组)")
            else:
                print(f"✗ 获取卡组返回非预期数据: {decks_data}")
                return False
        else:
    ...
    try:
        ...
        if 'deck_id' in locals() and deck_id:
            ...
            response = requests.post(f"{base_url}/api/local-game/start", headers=headers, json=game_data)
            if response.status_code == 200:
                ...
                if 'game_session_id' in game_start_data:
                    ...
                    # 测试获取游戏状态
                    response = requests.get(f"{base_url}/api/local-game/{game_session_id}/state", headers=headers)
                    if response.status_code == 200:
                        print("✓ 获取游戏状态测试通过")
                    else:
                    ...
                    # 结束游戏
                    response = requests.post(f"{base_url}/api/local-game/{game_session_id}/end", headers=headers, json={})
                    if response.status_code == 200:
                        print("✓ 结束游戏测试通过")
                    else:
